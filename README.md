...
8. The above should lead you to a page with the title "Node-RED in Bluemix for IBM Internet of Things Foundation". It has a big red button "Go to your NODE-RED flow editor" in the lower right. Click on it.

9. You should now be in your NODE-RED flow editor. It is populated with a few nodes. Mark them all with Ctrl-a and then delete them. We will start with a clean canvas.

10. Here is the flow that will connect your TI Sensor Tag with the IoT Foundation service. Select all of the JSON below and copy it into your clipboard.
...
11. Next import the flow into the Node-RED instance on Bluemix. This is done through: Menu --> Import --> Clipboard

![Importing from clipboard](import-from-clipboard.jpg)

12. You should now see the imported flow in your NODE-RED editor.

![Imported NODE RED Flow](screenshot-node-red-flow.jpg)

13. Once you have imported the flow you will need to double click on the IBM IoT node to open the configuration properties and replace the device ID with the device ID you registered your Sensor Tag with in Bluemix
